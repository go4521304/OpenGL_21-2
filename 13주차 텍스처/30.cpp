...
GLvoid drawScene()
{
	...
	glm::mat4 bg_mat(1.f);
	bg_mat = glm::scale(bg_mat, { 2.0f, 2.0f, 1.0f });
	glUniformMatrix4fv(modelMat, 1, GL_FALSE, glm::value_ptr(bg_mat));
...
void InitBuffer()
{
	...
	glBindVertexArray(vao_bg);
	glBindBuffer(GL_ARRAY_BUFFER, vbo_bg[0]);
	glBufferData(GL_ARRAY_BUFFER, vPos.size() * sizeof(glm::vec3), vPos.data(), GL_STATIC_DRAW);
	glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 3 * sizeof(float), 0);
	glEnableVertexAttribArray(0);

	glBindBuffer(GL_ARRAY_BUFFER, vbo_bg[1]);
	glBufferData(GL_ARRAY_BUFFER, vNormal.size() * sizeof(glm::vec3), vNormal.data(), GL_STATIC_DRAW);
	glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, 3 * sizeof(float), 0);
	glEnableVertexAttribArray(1);

	glBindBuffer(GL_ARRAY_BUFFER, vbo_bg[2]);
	glBufferData(GL_ARRAY_BUFFER, vTexCoord.size() * sizeof(glm::vec2), vTexCoord.data(), GL_STATIC_DRAW);
	glVertexAttribPointer(2, 2, GL_FLOAT, GL_FALSE, 2 * sizeof(float), 0);
	glEnableVertexAttribArray(2);
...
void InitTexture()
{
	int width[7], height[7], numberOfChannel[7];
	unsigned char* textureData[7];
	// 텍스처 읽기
	stbi_set_flip_vertically_on_load(true);
	textureData[0] = stbi_load("judy1.jpg", &width[0], &height[0], &numberOfChannel[0], 0);
	textureData[1] = stbi_load("judy2.jpg", &width[1], &height[1], &numberOfChannel[1], 0);
	textureData[2] = stbi_load("judy3.jpg", &width[2], &height[2], &numberOfChannel[2], 0);
	textureData[3] = stbi_load("molru3.jpg", &width[3], &height[3], &numberOfChannel[3], 0);
	textureData[4] = stbi_load("molru4.jpg", &width[4], &height[4], &numberOfChannel[4], 0);
	textureData[5] = stbi_load("molru5.jpg", &width[5], &height[5], &numberOfChannel[5], 0);
	textureData[6] = stbi_load("bg.jpg", &width[6], &height[6], &numberOfChannel[6], 0);
